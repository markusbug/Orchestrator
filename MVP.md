...
- ~~Android build. Same Flutter code, enabled once the iOS app is usable.~~ Runs on a real Android device as of 2026-09-08; a release-signed app bundle builds locally. Play Store listing is still out.
...
- **Attention flag.** Launch `claude` with `--settings <generated json>` that adds `UserPromptSubmit`, `Stop`, `StopFailure`, `Notification`, `PermissionRequest`, `PreToolUse` and `PostToolUse` hooks calling `<abs path>/orchestrator _hook <event>` over the local admin socket; the session id travels in `ORCHESTRATOR_SESSION_ID`, and the hook forwards `notification_type`, `tool_name` and `agent_id` from its stdin payload. Stop, StopFailure and an `idle_prompt` notification set *waiting* (reason `idle`); PermissionRequest, a `permission_prompt`/`elicitation_dialog` notification and PreToolUse of `AskUserQuestion` set *waiting* (reason `input`); UserPromptSubmit, other PreToolUse and PostToolUse set *running*. Hooks carrying an `agent_id` come from a background subagent and are ignored. Claude Code fires nothing on a user interrupt, so a bare Escape or Ctrl-C typed into a Claude session marks it *waiting* (`idle`); Enter, y or a digit while a prompt is pending marks it *running*; other typing changes nothing, and neither do replies the terminal emulator generates by itself (device attributes, cursor position, mouse, focus). The terminal bell is only a signal for sessions without hooks, and only when it is a real BEL rather than the terminator of an OSC title string.
