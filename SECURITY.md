...
**Releases.** Every release publishes `SHA256SUMS`, and `scripts/install.sh` fails
the install rather than proceeding when a download does not match, is missing from the
list, or the list is absent. That protects the transfer, not the release: the sums file
sits in the same release as the artifact, so anyone able to replace one can replace the
other. What covers the release itself is Sigstore build provenance, generated by the
release workflows and tied to the workflow, commit and runner that produced the file —
a credential that can only upload assets cannot mint it. Verify a download by hand with

```
gh attestation verify <file> -R markusbug/Orchestrator
```

or let the installer do it with `scripts/install.sh --require-provenance`. Provenance
needs a public repository, so it starts appearing with the first release published after
this repository goes public; releases before that carry checksums only. Workflows run
with `contents: read` except the jobs that publish, third-party actions are pinned to
commit SHAs, and the build tools the release jobs download are pinned and checksummed.
...
- **Protecting against a compromised GitHub account.** Provenance makes a forged
  release detectable by anyone who checks it, and pinning keeps a hijacked third-party
  action out of the release jobs. Neither stops someone who can push to `main` and run
  the workflows.
